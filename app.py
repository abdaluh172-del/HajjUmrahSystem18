...
import ai_pipeline          # v14: unified pipeline (LLM/moderation/relevance); v15: +category/mixed
...
# ---- v15: Google / Apple sign-in — PREPARED, not yet connected ----
# The spec asks the system to be READY to support these; without real OAuth
# client credentials there is nothing safe to verify, so these endpoints are
# honest stubs: they tell the frontend clearly that sign-in isn't configured
# yet instead of pretending to authenticate anyone. Wiring up the real flow
# later only means: verify the client's id_token against Google's public
# keys (or Apple's) using the configured client id, find-or-create the user
# by verified email, then return make_token(...) exactly like /auth/login.
...
def google_login():
    ...
    return api_error("Sign-in with Google isn't configured yet",
                     "تسجيل الدخول عبر Google غير مُفعّل بعد", 501, code="not_configured")
...
def dashboard_stats():
    ...
    mix = sum(1 for r in rows if r["sentiment"] == "mixed")  # v15
    neu = total - pos - neg - mix
    ...
    return jsonify({
        "total": total, "positive": pos, "negative": neg, "neutral": neu, "mixed": mix,
...
def ingest_external_comments():
    ...
    De-duplication happens in two layers:
      1. Same-platform re-fetch: each platform item carries its own id
         ...
         skipped, so refreshing hourly never creates duplicates.
    ...
    items = external_sources.fetch_all() + google_maps_source.fetch_reviews()
...
def analytics_overview():
    ...
    incl. mixed, category distribution, source comparison, country/city
    comparison (from Google Maps data), a daily sentiment trend for the
    last 30 days, and a light "suggestions" extraction (comments that read
    like a suggestion rather than a plain complaint/praise)."""
    db = get_db()
    ...
    for r in rows:
        for bucket, key in ((by_category, r["category"] or "general"),
            ...
            c = bucket.setdefault(key, {"total": 0, "positive": 0, "negative": 0, "neutral": 0, "mixed": 0})
            c["total"] += 1
            c[r["sentiment"]] = c.get(r["sentiment"], 0) + 1
        if r["place_country"]:
            c = by_country.setdefault(r["place_country"], {"total": 0, "positive": 0, "negative": 0, "neutral": 0, "mixed": 0})
            c["total"] += 1
            c[r["sentiment"]] = c.get(r["sentiment"], 0) + 1
        if r["place_city"]:
            c = by_city.setdefault(r["place_city"], {"total": 0, "positive": 0, "negative": 0, "neutral": 0, "mixed": 0})
            c["total"] += 1
            c[r["sentiment"]] = c.get(r["sentiment"], 0) + 1
    ...
    for r in rows:
        ...
        if not day or day < cutoff:
            continue
        d = trend.setdefault(day, {"positive": 0, "negative": 0, "neutral": 0, "mixed": 0, "total": 0})
        d["total"] += 1
